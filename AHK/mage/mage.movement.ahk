#SingleInstance, Force
#NoEnv
#MaxThreadsPerHotkey 10
#IfWinExist 
#Persistent

SetWinDelay, 0
SetBatchLines -1

Menu, Tray, Icon, omes.ico

;<><><><><><><><><><><><><><><><><><><><><><><><> Draw UI BUTTONS

MaxRange1 := 100 
typeface := "Nova Square"

ui_bitmap                       := Create_uiupdate_png()
ui_buttonon_bitmap              := Create_buttonon_png()
ui_buttonoff_bitmap             := Create_buttonoff_png()

ui_buttontoggle_bitmap          := Create_buttontoggle_png()
ui_buttontoggle_down_bitmap     := Create_buttontoggledown_png()

ui_buttonkillswitch_bitmap      := Create_buttonkillswitch_png()
ui_buttonkillswitch_down_bitmap := Create_buttonkillswitchdown_png()

ui_button_autopot_bitmap        := Create_buttonautopot_png()
ui_button_autopot_down_bitmap   := Create_buttonautopotdown_png()

Gui, Add, Picture, x0 y0 w415 h477 vgui_ui, % "HBITMAP:*" . ui_bitmap
Gui, Add, Picture, x40 y365 h25 w155 vgui_button_toggle gbuttontoggle_down, % "HBITMAP:*" . ui_buttontoggle_bitmap
Gui, Add, Picture, x220 y365 h25 w155 vgui_button_close gbuttonkill_down, % "HBITMAP:*" . ui_buttonkillswitch_bitmap
Gui, Add, Picture, x40 y395 h25 w155 vgui_button_autopot gbuttonautopot_down, % "HBITMAP:*" . ui_button_autopot_bitmap

;<><><><><><><><><><><><><><><><><><><><><><><><> Draw Font/text <><><><> Headers

Gui font, cC0C0C0 s9, Nova Square
Gui, Add, Text, x180 y13  BackgroundTrans, Navigation
Gui, Add, Text, x177 y225 BackgroundTrans, Movement
Gui, Add, Text, x192 y340 BackgroundTrans, Misc

;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> Draw Font/text Navigation tab

Gui, Add, Picture, x32 y36 vbuttonnav gbuttonnav_switch,  % "HBITMAP:*" . ui_buttonoff_bitmap         ; Start Navigation Y of text - 1
Gui, Add, Picture, x32 y166 vbuttonrune gbuttonrune_toggle,   % "HBITMAP:*" . ui_buttonoff_bitmap                        ;auto rune completion
Gui, Add, Picture, x32 y191 vbuttondc gbutton_dc,     % "HBITMAP:*" . ui_buttonoff_bitmap             ;chat dc

Gui, Add, Text, x50 y35 BackgroundTrans, Start Navigation (F6) 
Gui, Add, Text, x50 y55 BackgroundTrans, Map name
Gui, Add, Text, x50 y105 BackgroundTrans, Custom Map 

slidermapL := new CustomSlider(" x50 y125 w195 h9 c0x444444 c0x8bb53e", 50)   ;number berfore bitmap is width of screen box
slidermapR := new CustomSlider_reverse(" x50 y145 w195 h9 c0x444444 c0x8bb53e", 50)   ;number berfore bitmap is width of screen box
slidermapL.pos := 20 ; set new pos
slidermapR.pos := 20 ; set new pos

Gui, Add, Text, x250 y122 BackgroundTrans, Map Left 
Gui, Add, Text, x250 y142 BackgroundTrans, Map Right 

Gui, Add, Text, x50 y165 BackgroundTrans, AutoRune Completion 
Gui, Add, Text, x240 y165 BackgroundTrans, Interact Key 
Gui, Add, DDL,  x320 y161 w50 vDDLinteract, 1|2|3|4|5|6|7|8|9|0|q|w|e|r|t|y|u|i|o|p|a|s|d|f|g|h|j|k|l|z|x|c|v|b|n|m|Space

Gui, Add, Text, x50 y190 BackgroundTrans, Chat Disconnect (Safeguard) 

;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> MAPS

DDLContent =                                                              ;list made
(Join|
Arcana - Between Frost and Thunder 2
Copper Drakes
Ellinel Lake - Above the Lake 3
Five-Color Hill - Hill Path <1>
Kerning Tower - 3F <3>
Kerning Tower - 4F <4>
Lab - Area C-3
Lachelein - Chicken Festival <2>
Lachelein - Victory Plate <1>
Lake of Oblivion - Land of Sorrow
Ludibrium - Forgotten Passage
Slurpy Forest - Forest 1
Slurpy Forest - Forest depths
Twlight Perion - Site 2
custom map (small)
custom map (large)
)
Gui, Font, cC0C0C0 s9, Nova Square
Gui, Add, DDL, x50 y76 w250 vDDLselection, %DDLContent%                     ;creates the ui of ddl
GuiControlGet, Btn, Pos
GuiControlGet, DDL, Pos                                  ;DDL is dropdown list

Y := DDLY + ((BtnH - DDLH) // 2)                         ;BTN is ? H is height, can be used as btnX btnY BtnW BtnH
GuiControl, Move, DDL, y%Y%

;GuiControlGet, Outputvar, ,DDLselection,                ;Looks for the output variable of the DDL
;msgbox %Outputvar%                                      ;prints out the varible

GuiControl, MoveDraw, DDL ; needed to ensure that the control will shown properly after the first Gui, Show, ...
GuiControl, choose, DDLselection, small map navigation (side borders)

;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> version font

Gui font, c505050 s9, Nova Square
Gui, Add, Text, x308 y438 BackgroundTrans, Omes V.0.3.0                                      ;Version number

;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> toggle always ontop
Gui font, cC0C0C0 s9, Nova Square
Gui, Add, Text, x50 y432 BackgroundTrans, Always ontop
Gui, Add, Picture, x32 y433 vbuttonontop gbuttonontop_switch,  % "HBITMAP:*" . ui_buttonoff_bitmap 

;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> Draw Font/text Skills

Gui font, cC0C0C0 s9, Nova Square

Gui, Add, Picture, x32 y251 vbuttonmovement gbuttonmovement_switch, % "HBITMAP:*" . ui_buttonoff_bitmap
Gui, Add, Text, x50 y250 BackgroundTrans, Enable Movement (F7)
Gui, Add, Text, x50 y270 BackgroundTrans, Movement type
Gui, Add, Text, x200 y270 BackgroundTrans, Movement Key
Gui, Add, Text, x311 y270 BackgroundTrans, Delay (MS)

Gui, Add, DDL, x50 y290 w145 vDDLmovement, Single_Jump|Double_Jump_Fast|Double_Jump_Slow|Teleport
Gui, Add, DDL, x200 y290 w100 vDDLmovement_key, Space|q|w|e|r|t|a|s|d|f|g|z|x|c|v

Gui font, cBlack s9, Nova Square
Gui, Add, edit, x320 y290 w50 r1 vMovement_delay, 1000

GuiControl, choose, DDLmovement, Single_Jump

OnMessage(WM_MOUSEWHEEL:=0x20A, "wheel")
OnMessage(WM_MOUSEMOVE:=0x200, "drag")
return


drag(wParam, lParam)
{
   static yp,lastControl,change
   sensitivity:=7 ; In pixels, about how far should the mouse move before adjusting the value?
   amt:=1         ; How much to increase the value
   mode:=1        ; 1 = up/down, 2=left/right
   
   ; some safety checks
   if (!GetKeyState("Lbutton", "P"))
      return
   GuiControlGet, controlType, Focus
   if (!instr(controlType, "Edit"))
      return
   GuiControlGet, value,, %A_GuiControl%
   if value is not number
      return
 
   if (mode=1)
      MouseGetPos,, y
   else if (mode=2)
   {
      MouseGetPos, y
      y*=-1 ; need to swap it so dragging to the right adds, not subtracts.
   }
   else
      return
      
   if (lastControl!=A_GuiControl) ; set the position to the current mouse position
      yp:=y
   change:=abs(y-yp)              ; check to see if the value is ready to be changed, has it met the sensitivity?
   
   mult:=((wParam=5) ? 0.01 : (wParam=9) ? 0.1 : 1)
   value+=((y<yp && change>=sensitivity) ? amt*mult : (y>yp && change>=sensitivity) ? -amt*mult : 0)
 
   GuiControl,, %A_GuiControl%, % RegExReplace(value, "(\.[1-9]+)0+$", "$1")
   
   if (change>=sensitivity)
      yp:=y
   lastControl:=A_GuiControl
}
 
wheel(wParam, lParam)
{
   amt:=1 ; How much to increase the value
   GuiControlGet, controlType, Focus
   if (!instr(controlType, "Edit"))
      return
      
   GuiControlGet, value,, %A_GuiControl%
   if value is not number
      return
   
   mult:=((wParam & 0xffff=4) ? 0.01 : (wParam & 0xffff=8) ? 0.1 : 1)
   value+=((StrLen(wParam)>7) ? -amt*mult : amt*mult)
   GuiControl,, %A_GuiControl%, % RegExReplace(value, "(\.[1-9]+)0+$", "$1")
}
return

;<><><><><><><><><><><><><><><><><><><><><><><><> Character inputs / loop variables


navigation := 0
menu := 0
autorune := 0

;<><><><><><><><><><><><><><><><><><><><><><><><> Start loop

*F5:: 


;<><><><><><><><><><><><><><><><><><><><><><><><> Character inputs / loop variables
;mapsearch_x1 := 10
;mapsearch_y1 := 84
;mapsearch_x2 := 185
;mapsearch_y2 := 152
pathing_upper := 0
pathing_lower := 0
arrow_red_id    := "0xE4601F|0xDD3A15|0xE32915|0xD34E16|0xCE5715|0xE70A15|0xBD7751|0xD93815|0xAD7A4A|0x0ADA3E|0xA83846|0xCC7139|0xB87A3D|0xD7633E|0xB17C36|0xE18815|0xB86811"
arrow_green_id  := "0x02E043|0x4DCC7D|0x04f115|0x02D162|0x43c665|0x16ef43|0x4BC487|0x43C743|0x51CB85|0x51CB85|0x64CB4D|0x0ADA3E|0x36BB74|0x00DD66|0x3EFF3B"
scan_x := 450
arrowvalue := 0
rune_execute := 0
looping_arrows = 0

togglemenu:=!togglemenu				
	if (!togglemenu)
	{
		menu = 0
		Gui, Hide
	}
	else
	{
		Gui, Show, x1500 y0 w415 h477
		menu = 1
	}
	
;<><><><><><><><><><><><><><><><><><><><><><><><> Start Timer checks

SetTimer, menu_open, % (togglemenucheck:=!togglemenucheck) ? 100 : "Off"
menu_open:

	if (!togglemenucheck)
	{
		Gui, Hide
		return
	}
	
	else if (menu = 1)
	{
		GuiControlGet, Delayvar, , Movement_delay,
		GuiControlGet, Movementvar, ,DDLmovement,
		GuiControlGet, Outputvar, ,DDLselection,
		GuiControlGet, interactkeyvar, ,DDLinteract,
		
		movement_class = %Movementvar%

		Random, 10ms, 9,14
		Random, 100ms, 98, 124
		Random, 200ms, 202, 210
		Random, 300ms, 297, 305
		Random, 400ms, 411, 435
		Random, 500ms, 497, 521
		Random, 800ms, 801, 814


;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><<><><><><><><><><><><><><><><><>><><> dc check

		if (dc = 1)
		{
			pixelsearch ,,, 8, 761, 103, 779, 0xe8e6e3, 4, Fast Rgb
			if !ErrorLevel
			{
				click 956, 762
				sleep 100ms
				click 949, 720
				sleep 100ms
				click 650, 526
			}
		}
		
;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><<><><><><><><><><><><><><><><><>><><> autorune

		if (autorune = 1)
		{
			PixelSearch, rune_x, rune_y, mapsearch_x1, mapsearch_y1, mapsearch_x2, mapsearch_y2, 0xdd66ff, 2, fast RGB  
			if !Errorlevel
			{
				soundbeep 500,500
				goto buttonrune_toggle
			}
			
			PixelSearch, rune_x, rune_y, mapsearch_x1, mapsearch_y1, mapsearch_x2, mapsearch_y2, 0xdd66ff, 2, fast RGB  
			if (!Errorlevel and rune_execute = 99)
			{
				rune_execute = 1
				
				;<><><><><><><><><><><><><><><><><><><><> map out quadrants
				quad_right_x1 = % rune_x + 4
				quad_right_y1 = %mapsearch_y1%
				quad_right_x2 = %mapsearch_x2%
				quad_right_y2 = %mapsearch_y2%
				
				quad_up_x1 = % rune_x - 4
				quad_up_y1 = %mapsearch_y1%
				quad_up_x2 = % rune_x + 4
				quad_up_y2 = % rune_y - 10
				
				quad_left_x1 = %mapsearch_x1%
				quad_left_y1 = %mapsearch_y1%
				quad_left_x2 = % rune_x - 5
				quad_left_y2 = %mapsearch_y2%
				
				quad_down_x1 = % rune_x - 4
				quad_down_y1 = % rune_y + 10
				quad_down_x2 = % rune_x + 4
				quad_down_y2 = %mapsearch_y2%
				
				on_rune_x1 = % rune_x - 2
				on_rune_y1 = % rune_y - 2
				on_rune_x2 = % rune_x + 2
				on_rune_y2 = % rune_y + 2
			}
			
			if (rune_execute = 1 and looping_arrows = 0)
			{
			
				PixelSearch,,, mapsearch_x1, mapsearch_y1, mapsearch_x2, mapsearch_y2, 0xFFDD44, 2, fast RGB 
				if !Errorlevel
				{
					PixelSearch,,, quad_right_x1, quad_right_y1, quad_right_x2, quad_right_y2, 0xFFDD44, 2, fast RGB  ;quad 1
					if (!Errorlevel and looping_arrows = 0)
					{
						Sendinput {Blind}{Right up}
						Sendinput {Blind}{Down up}
						Sendinput {Blind}{Up up}
						Sendinput {Left down}
					}
					PixelSearch,,, quad_up_x1, quad_up_y1, quad_up_x2, quad_up_y2, 0xFFDD44, 2, fast RGB  ;quad 2
					if (!Errorlevel and looping_arrows = 0)
					{
						Sendinput {Blind}{Left up}
						Sendinput {Blind}{Down up}
						Sendinput {Blind}{Up up}
						Sendinput {down down}
						;Sendinput {Alt down}
						;Sleep 100ms
						;Sendinput {Blind}{Alt up}
					}
					PixelSearch,,, quad_left_x1, quad_left_y1, quad_left_x2, quad_left_y2, 0xFFDD44, 2, fast RGB  ;quad 3
					if (!Errorlevel and looping_arrows = 0)
					{
						Sendinput {Blind}{Left up}
						Sendinput {Blind}{Down up}
						Sendinput {Blind}{Up up}
						Sendinput {Right down}
					}
					PixelSearch,,, quad_down_x1, quad_down_y1, quad_down_x2, quad_down_y2, 0xFFDD44, 2, fast RGB  ;quad 4
					if (!Errorlevel and looping_arrows = 0)
					{
						Sendinput {Blind}{Left up}
						Sendinput {Blind}{Right up}
						Sendinput {Blind}{Down up}
						Sendinput {Up down}
						%movement_class%()
						Sleep 100ms
					}
					
					PixelSearch,,, on_rune_x1, on_rune_y1, on_rune_x2, on_rune_y2, 0xFFDD44, 2, fast RGB  ;on rune
					if (!Errorlevel and looping_arrows = 0)
					{
						Sendinput {Blind}{Left up}
						Sendinput {Blind}{Right up}
						Sendinput {Blind}{Up up}
						Sendinput {Blind}{Down up}
						sleep 500
						Sendinput {%interactkeyvar%}
						
						Loop, parse, arrow_red_id, |,
						{
							looping_arrows = 1
							
							PixelSearch, found_red_x, found_red_y, 430, 200, 600, 312, %A_Loopfield%, 20, fast RGB        ;finds the red arrows starting from left to right
							if !Errorlevel 
							{
								pixelgetcolor, red_arrow, found_red_x, found_red_y, rgb
								arrowvalue = 0
								
								scan_x1 = % found_red_x - 40
								scan_x2 = % found_red_x + 40

								;mousemove, %scan_x1%, 200
								;mousemove, %scan_x2%, 312
								
								Loop, parse, arrow_green_id, |,
								{
									PixelSearch,found_green_x, found_green_y, 430, 200, 600, 312, %A_Loopfield%, 20, Fast RGB
									if !Errorlevel
									{
										pixelgetcolor, green_arrow, found_green_x, found_green_y, rgb

										scanning:
										Loop, parse, arrow_red_id, |,
										{	
											PixelSearch, found_arrow_x, found_arrow_y, %scan_x1%, 200, %scan_x2%, 312, %A_Loopfield%, 25, Fast RGB
											
											;Mousemove, %scan_x1%, 200                           ;watch progress bar scan to the right
											;Mousemove, %scan_x2%, 312   
											
											if !Errorlevel
											{
												found_arrow_x_left  := found_arrow_x-8
												found_arrow_x_right := found_arrow_x+8
												found_arrow_y_up    := found_arrow_y-10
												found_arrow_y_down  := found_arrow_y+10
												
												scanning_down_y     := found_arrow_y+35
												scanning_up_y       := found_arrow_y-35
												scanning_right_x    := found_arrow_x+35
												scanning_left_x     := found_arrow_x-35


												Loop, parse, arrow_green_id, |,
												{
													PixelSearch,,, %found_arrow_x_left%, %found_arrow_y%, %found_arrow_x_right%, %scanning_down_y%, %A_Loopfield%, 15, Fast RGB       ;Search down (UP)  
														
													;MouseMove, %found_arrow_x_left%,%found_arrow_y%
													;MouseMove, %found_arrow_x_right%, %scanning_down_y%
													
													if !Errorlevel
													{
														Sendinput {Up}
														sleep 1000
														msgbox, up
														arrowvalue++
															scan_x1 = % scan_x1+100
															scan_x2 = % scan_x2+100
														goto scanning
													}
													
													PixelSearch,,, %found_arrow_x_left%, %found_arrow_y%, %found_arrow_x_right%, %scanning_up_y%, %A_Loopfield%, 15, Fast RGB        ;Search up (DOWN)  
														
													;MouseMove, %found_arrow_x_left%, %found_arrow_y%
													;MouseMove, %found_arrow_x_right%, %scanning_up_y%
													
													if !Errorlevel
													{
														Sendinput {Down}
														sleep 1000
														msgbox, down
															scan_x1 = % scan_x1+100
															scan_x2 = % scan_x2+100
														arrowvalue++
														goto scanning
													}
													
													PixelSearch,,, %found_arrow_x%, %found_arrow_y_up%, %scanning_right_x%, %found_arrow_y_down%, %A_Loopfield%, 15, Fast RGB        ;Search right (LEFT)  
														
													;MouseMove, %found_arrow_x%, %found_arrow_y_up%
													;MouseMove, %scanning_right_x%, %found_arrow_y_down%
													
													if !Errorlevel
													{
														
														Sendinput {Left}
														sleep 1000
														msgbox, left
															scan_x1 = % scan_x1+100
															scan_x2 = % scan_x2+100
														arrowvalue++
														goto scanning
													}
												
														
													PixelSearch,,, %found_arrow_x%, %found_arrow_y_up%, %scanning_left_x%, %found_arrow_y_down%, %A_Loopfield%, 15, Fast RGB          ;Search left (Right)   
														
													;MouseMove, %found_arrow_x%, %found_arrow_y_up%
													;MouseMove, %scanning_left_x%, %found_arrow_y_down%
													
													if !Errorlevel
													{
														Sendinput {Right}
														sleep 1000
														msgbox, right 
															scan_x1 = % scan_x1+100
															scan_x2 = % scan_x2+100
														arrowvalue++
														goto scanning
													}
												}until arrowvalue = 4
											}
										}until arrowvalue = 4
									}
								}
							}
							if (arrowvalue = 4)
							{
								looping_arrows = 0
								rune_execute = 0
							}
						}
					}
				}
			}
		}

;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><<><><><><><><><><><><><><><><><>><><> Movement
		if (movement = 1 and rune_execute = 0) 
		{
			if (Movementvar = "Single_Jump")
			{
				%movement_class%()
				Sleep %Delayvar%
				Sleep 10ms
			}
			if (Movementvar = "Double_Jump_Fast")
			{
				%movement_class%()
				Sleep %Delayvar%
				Sleep 10ms
			}
			if (Movementvar = "Double_Jump_Slow")
			{
				%movement_class%()
				Sleep %Delayvar%
				Sleep 10ms
			}
			if (Movementvar = "Teleport")
			{
				%movement_class%()
				Sleep %Delayvar%
				Sleep 10ms
			}
		}
		
		
;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><<><><><><><><><><><><><><><><><>><><> small map nagivation with side borders
		if (navigation = 1 and rune_execute = 0) 
		{
			
			if (navigation = 1 and Outputvar = "Arcana - Between Frost and Thunder 2") 
			{
				mapsearch_x1 := 11
				mapsearch_y1 := 87
				mapsearch_x2 := 221
				mapsearch_y2 := 137
				
				PixelSearch,,, 11, 87, 50, 117, 0xFFDD44, 2, Fast RGB     ;Left side start moving right
				if !ErrorLevel
				{
					pathing_upper = 0
					pathing_lower = 0
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
				
				PixelSearch,,, 115, 92, 130, 102, 0xFFDD44, 2, Fast RGB     ;if tp to platform above
				if !ErrorLevel
				{
					pathing_upper = 1
					pathing_lower = 0
				}
				PixelSearch,,, 88, 111, 125, 119, 0xFFDD44, 2, Fast RGB     ;if tp to platform below
				if !ErrorLevel
				{
					if pathing_lower = 0
					{
						pathing_lower = 1
						pathing_upper = 0
						sleep 3000
					}
					else if pathing_upper = 1
					{
						pathing_lower = 1
						pathing_upper = 0
					}
				}
				PixelSearch,,, 150, 89, 179, 100, 0xFFDD44, 2, Fast RGB     ;top platform end
				if !ErrorLevel
				{
					if (pathing_lower = 1 and pathing_upper = 0)
					{
						Sendinput {Blind}{Up up}
						Sendinput {Blind}{Right up}
						Sendinput {Left Down}
					}
				}
				PixelSearch,,, 160, 89, 179, 100, 0xFFDD44, 2, Fast RGB     ;top platform end
				if !ErrorLevel
				{
					if (pathing_upper = 1 and pathing_lower = 0)
					{
						Sendinput {Blind}{Right up}
						Sendinput {Down down}
					}
				}
				PixelSearch,,, 150, 104, 180, 117, 0xFFDD44, 2, Fast RGB     ;bottom platform end
				if !ErrorLevel 
				{
					if (pathing_upper = 1 and pathing_lower = 0)
					{
						Sendinput {Blind}{Down up}
						Sendinput {Blind}{Right up}
						Sendinput {Blind}{Up up}
						Sendinput {Left down}
					}
				}
				PixelSearch,,, 160, 104, 180, 117, 0xFFDD44, 2, Fast RGB     ;bottom end
				if !ErrorLevel
				{
					if (pathing_lower = 1 and pathing_upper = 0)
					{
						Sendinput {Blind}{Right up}
						Sendinput {Blind}{Down up}
						Sendinput {Up Down}
						msgbox, bottom
					}
				}
				
				PixelSearch,,, 183, 92, 220, 120, 0xFFDD44, 2, Fast RGB     ;far right
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Down up}
					Sendinput {Blind}{Up up}
					Sendinput {Left down}
				}
			}
			
			if (navigation = 1 and Outputvar = "Ellinel Lake - Above the Lake 3") 
			{
				PixelSearch,,, 143, 80, 179, 135, 0xFFDD44, 2, Fast RGB     ;right side
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
		
				PixelSearch,,, 11, 80, 35, 135, 0xFFDD44, 2, Fast RGB        ;left side
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
			}
			
			if (navigation = 1 and Outputvar = "Copper drakes")
			{
				PixelSearch,,, 62, 96, 94, 110, 0xFFDD44, 3, Fast RGB  ;4th Level move left
				if !ErrorLevel
				{	
					Sendinput {Blind}{Up up}
					Sendinput {down down}
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}

				PixelSearch,,, 42, 96, 61, 110, 0xFFDD44, 3, Fast RGB  ;4th Level jump down
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Alt down}
					Sendinput {Blind}{Alt up}
				}
				
				PixelSearch,,, 38, 120, 78, 131, 0xFFDD44, 3, Fast RGB  ;3rd Level move right
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Alt up}
					Sendinput {Right down}
				}
				
				PixelSearch,,, 79, 120, 94, 131, 0xFFDD44, 3, Fast RGB  ;3rd Level jump down
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Alt down}
					Sleep 100ms
					Sendinput {Blind}{Alt up}
				}
				
				PixelSearch,,, 54, 136, 94, 145, 0xFFDD44, 3, Fast RGB  ;2nd Level move left
			
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Alt up}
					Sendinput {Left down}
				}
				
				PixelSearch,,, 38, 136, 53, 145, 0xFFDD44, 3, Fast RGB  ;2nd Level jump down
			
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Alt down}
					Sendinput {Blind}{Alt up}
				}
				
				PixelSearch,,, 40, 156, 70, 158, 0xFFDD44, 3, Fast RGB  ;1st Level move right
			
				if !ErrorLevel
				{
					Sendinput {Blind}{Alt up}
					Sendinput {Blind}{down up}
					Sendinput {Blind}{Left up}
					Sendinput {Up down}
					Sendinput {Right down}
				}
				
				PixelSearch,,, 71, 156, 106, 164, 0xFFDD44, 3, Fast RGB  ;1st Level move left
			
				if !ErrorLevel
				{
					Sendinput {Blind}{Alt up}
					Sendinput {Blind}{down up}
					Sendinput {Blind}{Right up}
					Sendinput {Up down}
					Sendinput {Left down}
				}
				PixelSearch,,, 20, 143, 40, 157, 0xFFDD44, 3, Fast RGB  ;out of bounds left
			
				if !ErrorLevel
				{
					Sendinput {Blind}{Alt up}
					Sendinput {Blind}{down up}
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
				
			}
			if (navigation = 1 and Outputvar = "Five-Color Hill - Hill Path <1>")
			{
			
				PixelSearch,,, 11, 117, 118, 149, 0xFFDD44, 2, Fast RGB     ;Bottom left side
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{down up}
					Sendinput {Right down}
				}

		
				PixelSearch,,, 119, 135, 165, 152, 0xFFDD44, 2, Fast RGB        ;below third platform
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {up down}
					
				}
				
				PixelSearch,,, 121, 116, 147, 136, 0xFFDD44, 2, Fast RGB        ;bottom third platform
				if !ErrorLevel
				{
					Sendinput {Blind}{up up}
					Sendinput {Right down}
					
				}
				PixelSearch,,, 167, 135, 181, 154, 0xFFDD44, 2, Fast RGB        ;too far bottom right
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Up up}
					Sendinput {Left down}
					
				}
				PixelSearch,,, 152, 113, 173, 130, 0xFFDD44, 2, Fast RGB        ;teleport to top platform
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Right up}
					Sendinput {up down}
					
				}
				PixelSearch,,, 174, 113, 182, 130, 0xFFDD44, 2, Fast RGB        ;Face left if too far
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
					
				}
				PixelSearch,,, 34, 87, 177, 109, 0xFFDD44, 2, Fast RGB        ;Top platform
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Up up}
					Sendinput {Left down}
					
				}
				PixelSearch,,, 10, 87, 35, 109, 0xFFDD44, 2, Fast RGB        ;Top platform
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Left up}
					Sendinput {Down down}
					
					Sendinput {Alt down}
					Sleep 100ms
					Sendinput {Blind}{Alt up}
					
				}
			}
			if (navigation = 1 and Outputvar = "Kerning Tower - 3F <3>") 
			{
				
				PixelSearch,,, 49, 133, 68, 143, 0xFFDD44, 2, Fast RGB     ;left side bottom
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Right down}
				}
				PixelSearch,,, 69, 133, 89, 143, 0xFFDD44, 2, Fast RGB     ;left side bottom
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Right up}
					Sendinput {Up down}
				}
		
				PixelSearch,,, 50, 92, 92, 117, 0xFFDD44, 2, Fast RGB        ;left side top
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
				
				PixelSearch,,, 89, 133, 196, 148, 0xFFDD44, 2, Fast RGB        ;move left 1st floor
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
				PixelSearch,,, 196, 133, 217, 148, 0xFFDD44, 2, Fast RGB        ;bottom right move left 1st floor
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
			}
			if (navigation = 1 and Outputvar = "Kerning Tower - 4F <4>")
			{
			
				PixelSearch,,, 130, 89, 164, 123, 0xFFDD44, 2, Fast RGB     ;Right side jump down
				if !ErrorLevel
				{
					Sendinput {Right down}
					Sendinput {Down down}

				}
				
				PixelSearch,,, 119, 123, 164, 145, 0xFFDD44, 2, Fast RGB     ;Right side down start moving left
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Down up}
					Sendinput {Left down}
				}
				
				PixelSearch,,, 9, 121, 30, 144, 0xFFDD44, 2, Fast RGB     ;Left side teleport too far move back right
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
				
				PixelSearch,,, 67, 134, 101, 147, 0xFFDD44, 2, Fast RGB     ;Left side teleport too far move back right adjust
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
				
				PixelSearch,,, 32, 131, 51, 145, 0xFFDD44, 2, Fast RGB     ;Sweet spot
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Left up}
					Sendinput {Up down}
				}
				
				PixelSearch,,, 11, 90, 52, 112, 0xFFDD44, 2, Fast RGB     ;Top move left
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Up up}
					Sendinput {Right down}
				}
				
			}
			if (navigation = 1 and Outputvar = "Lab - Area C-3")
			{
			
				PixelSearch,,, 150, 89, 166, 120, 0xFFDD44, 2, Fast RGB     ;right side top
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{up up}
					Sendinput {Left down}
					Sendinput {Down down}
				}
				PixelSearch,,, 130, 124, 166, 144, 0xFFDD44, 2, Fast RGB     ;right side bottom
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
		
				PixelSearch,,, 53, 89, 135, 115, 0xFFDD44, 2, Fast RGB        ;left side top
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
					
				}
				PixelSearch,,, 53, 125, 84, 138, 0xFFDD44, 2, Fast RGB        ;left side bottom
				if !ErrorLevel
				{
					Sendinput {Blind}{Down up}
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
					Sendinput {Up down}
					
					Sendinput {Alt down}
					Sleep 100ms
					Sendinput {Blind}{Alt up}
				}
				PixelSearch,,, 87, 125, 125, 138, 0xFFDD44, 2, Fast RGB        ;left side bottom right
				if !ErrorLevel
				{
					Sendinput {Blind}{Down up}
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
					Sendinput {Up down}
				}
				
			}
			
			if (navigation = 1 and Outputvar = "Lake of Oblivion - Land of Sorrow")
			{
			
				PixelSearch,,, 42, 92, 65, 157, 0xFFDD44, 2, Fast RGB     ;Left move right
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Up up}
					Sendinput {Blind}{Down up}
					Sendinput {Right down}
				}
				PixelSearch,,, 66, 108, 91, 141, 0xFFDD44, 2, Fast RGB     ;Left 2nd and 3rd platform
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Left up}
					Sendinput {Up down}
				}
				PixelSearch,,, 90, 146, 173, 157, 0xFFDD44, 2, Fast RGB     ;1st floor drop down
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
				PixelSearch,,, 67, 146, 89, 157, 0xFFDD44, 2, Fast RGB     ;1st floor drop down
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Right up}
					Sendinput {Up down}
				}
				PixelSearch,,, 66, 86, 102, 103, 0xFFDD44, 2, Fast RGB     ;Leftside top platform
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Up up}
					Sendinput {Right down}
				}
				
				PixelSearch,,, 162, 95, 173, 114, 0xFFDD44, 2, Fast RGB     ;Rightside top platform
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Down down}
				}
				
				PixelSearch,,, 159, 117, 173, 127, 0xFFDD44, 2, Fast RGB     ;Rightside 3rd platform
				if !ErrorLevel
				{
					Sendinput {Blind}{Down up}
					Sendinput {Blind}{Left up}
					Sendinput {Left down}
				}
				
				PixelSearch,,, 131, 117, 158, 127, 0xFFDD44, 2, Fast RGB     ;Rightside 3rd platform
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Up up}
					Sendinput {Down down}
				}
				PixelSearch,,, 124, 130, 163, 140, 0xFFDD44, 2, Fast RGB     ;Rightside 2nd
				if !ErrorLevel
				{
					Sendinput {Blind}{Down up}
					Sendinput {Blind}{Up up}
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
				
				PixelSearch,,, 164, 130, 175, 140, 0xFFDD44, 2, Fast RGB     ;Rightside 2nd
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Up up}
					Sendinput {Blind}{Left up}
					Sendinput {down down}
				}
				
				PixelSearch,,, 127, 146, 174, 158, 0xFFDD44, 2, Fast RGB     ;Bottom right
				if !ErrorLevel
				{
					Sendinput {Blind}{Down up}
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Up up}
					Sendinput {Left down}
				}
			}
			
			if (navigation = 1 and Outputvar = "Ludibrium - Forgotten Passage")
			{
			
				PixelSearch,,, 14, 129, 43, 174, 0xFFDD44, 2, Fast RGB     ;Left side bottom portal
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
					Sendinput {Up down}
				}
				PixelSearch,,, 44, 129, 176, 174, 0xFFDD44, 2, Fast RGB     ;Right side to portal
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
				PixelSearch,,, 31, 90, 174, 122, 0xFFDD44, 2, Fast RGB     ;Top platform
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
				PixelSearch,,, 175, 92, 196, 149, 0xFFDD44, 2, Fast RGB     ;Right side drop down
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
			}
			if (navigation = 1 and Outputvar = "Lachelein - Chicken Festival <2>")
			{
				mapsearch_x1 := 24
				mapsearch_y1 := 88
				mapsearch_x2 := 171
				mapsearch_y2 := 130
				
				PixelSearch,,, 24, 92, 42, 122, 0xFFDD44, 2, Fast RGB     ;left too far
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
				PixelSearch,,, 43, 107, 62, 124, 0xFFDD44, 2, Fast RGB     ;left teleport up
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Left up}
					Sendinput {Up down}
				}
				PixelSearch,,, 37, 88, 147, 100, 0xFFDD44, 2, Fast RGB     ;top left move right
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
				PixelSearch,,, 148, 88, 171, 100, 0xFFDD44, 2, Fast RGB     ;right teleport down
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Left up}
					Sendinput {Down down}
				}
				PixelSearch,,, 65, 107, 171, 130, 0xFFDD44, 2, Fast RGB     ;Right side bottom
				if !ErrorLevel
				{
					Sendinput {Blind}{Down up}
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Up up}
					Sendinput {Left down}
				}
				
			}
			if (navigation = 1 and Outputvar = "Lachelein - Victory Plate <1>")
			{
				mapsearch_x1 := 40
				mapsearch_y1 := 93
				mapsearch_x2 := 156
				mapsearch_y2 := 153
				
				PixelSearch,,, 40, 126, 46, 152, 0xFFDD44, 2, Fast RGB     ;Bottom level move right slightly
				if !ErrorLevel
				{
					Sendinput {Blind}{Down up}
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
				PixelSearch,,, 46, 126, 59, 152, 0xFFDD44, 2, Fast RGB     ;Bottom level move right slightly
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Right up}
					Sendinput {Up down}
				}
				PixelSearch,,, 60, 126, 160, 152, 0xFFDD44, 2, Fast RGB     ;Bottom level move right slightly
				if !ErrorLevel
				{
					Sendinput {Blind}{Down up}
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
				PixelSearch,,, 45, 104, 69, 120, 0xFFDD44, 2, Fast RGB     ;Left side second level move right
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Right down}
				}
				PixelSearch,,, 70, 108, 104, 124, 0xFFDD44, 2, Fast RGB     ;Left side second level teleport up
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Up down}
				}
				PixelSearch,,, 52, 84, 120, 101, 0xFFDD44, 2, Fast RGB     ;Top level move right
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Right down}
				}
				PixelSearch,,, 121, 84, 148, 101, 0xFFDD44, 2, Fast RGB     ;Top level teleport down
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Down down}
				}
				PixelSearch,,, 114, 110, 148, 123, 0xFFDD44, 2, Fast RGB     ;Second level right side move right
				if !ErrorLevel
				{
					Sendinput {Blind}{Down up}
					Sendinput {Right down}
				}
				PixelSearch,,, 149, 110, 154, 123, 0xFFDD44, 2, Fast RGB     ;Second level right side move right
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Down	down}
				}
			}
			
			if (navigation = 1 and Outputvar = "Slurpy Forest - Forest 1")
			{
			
				mapsearch_x1 := 13
				mapsearch_y1 := 88
				mapsearch_x2 := 185
				mapsearch_y2 := 146
			
				PixelSearch,,, 13, 122, 129, 146, 0xFFDD44, 2, Fast RGB     ;bottom move left
				if !ErrorLevel
				{	
					
					Sendinput {Blind}{Down up}
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
				PixelSearch,,, 130, 122, 162, 146, 0xFFDD44, 2, Fast RGB        ;bottom teleport up
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Right up}
					Sendinput {Up down}
					
				}
				PixelSearch,,, 163, 122, 185, 146, 0xFFDD44, 2, Fast RGB        ;bottom right side too far
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Up up}
					Sendinput {Left down}
				}
				PixelSearch,,, 33, 88, 182, 119, 0xFFDD44, 2, Fast RGB        ;Top side move left
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Up up}
					Sendinput {Left down}
				}
				PixelSearch,,, 15, 88, 32, 119, 0xFFDD44, 2, Fast RGB        ;Topside jump down
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Down down}
					%movement_class%()
					sleep 100ms
					Sendinput {Blind}{Down up}
				}
			}	
			if (navigation = 1 and Outputvar = "Slurpy Forest - Forest depths")
			{
				mapsearch_x1 := 14
				mapsearch_y1 := 96
				mapsearch_x2 := 148
				mapsearch_y2 := 160
				
				PixelSearch,,, 14, 100, 90, 154, 0xFFDD44, 2, Fast RGB     ;bottom too far left
				if !ErrorLevel
				{	
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Up up}
					Sendinput {Right down}
				}
				PixelSearch,,, 66, 111, 80, 154, 0xFFDD44, 2, Fast RGB     ;bottom left teleport up
				if !ErrorLevel
				{	
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Left up}
					Sendinput {Up Down}
				}
				PixelSearch,,, 66, 126, 80, 154, 0xFFDD44, 2, Fast RGB     ;bottom left teleport up
				if !ErrorLevel
				{	
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Left up}
					Sendinput {Up Down}
				}
				PixelSearch,,, 73, 96, 103, 107, 0xFFDD44, 2, Fast RGB     ;top move right
				if !ErrorLevel
				{	
					Sendinput {Blind}{Left up}
					Sendinput {Right Down}
				}
				PixelSearch,,, 104, 96, 111, 139, 0xFFDD44, 2, Fast RGB     ;top move right teleport down
				if !ErrorLevel
				{	
					Sendinput {Blind}{Up up}
					Sendinput {Blind}{Right up}
					Sendinput {Blind}{Left up}
					Sendinput {Down down}
					%movement_class%()
					sleep 100ms
					Sendinput {Blind}{Down up}
				}
				PixelSearch,,, 122, 96, 135, 137, 0xFFDD44, 2, Fast RGB     ;Right side move left platform
				if !ErrorLevel
				{	
					Sendinput {Blind}{Right up}
					Sendinput {Left Down}
				}
				PixelSearch,,, 81, 140, 148, 160, 0xFFDD44, 2, Fast RGB     ;Right side move left platform
				if !ErrorLevel
				{	
					Sendinput {Blind}{Down up}
					Sendinput {Blind}{Right up}
					Sendinput {Left Down}
				}
			}
			if (navigation = 1 and Outputvar = "Twlight Perion - Site 2")
			{
			
				PixelSearch,,, 149, 92, 211, 155, 0xFFDD44, 2, Fast RGB     ;right side
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
		
				PixelSearch,,, 102, 111, 148, 138, 0xFFDD44, 2, Fast RGB        ;right side center
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Blind}{Right up}
					Sendinput {up down}
				}
				PixelSearch,,, 102, 94, 153, 110, 0xFFDD44, 2, Fast RGB        ;right side center top
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Left down}
				}
				PixelSearch,,, 12, 87, 73, 141, 0xFFDD44, 2, Fast RGB        ;Leftside 
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
				PixelSearch,,, 72, 99, 100, 124, 0xFFDD44, 2, Fast RGB        ;Leftside top center
				if !ErrorLevel
				{
					Sendinput {Blind}{Up up}
					Sendinput {Blind}{Left up}
					Sendinput {Down down}
				}
				PixelSearch,,, 66, 127, 109, 142, 0xFFDD44, 2, Fast RGB        ;Leftside center
				if !ErrorLevel
				{
					Sendinput {Blind}{down up}
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
			}	
			if (navigation = 1 and Outputvar = "custom map (small)")
			{
				custom_map_left = % slidermapL.pos + 12
				y_value := slidermapR.pos
				custom_map_right := 180 - y_value
			
				PixelSearch,,, %custom_map_right%, 80, 151, 140, 0xFFDD44, 2, Fast RGB     ;right side
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
		
				PixelSearch,,, 12, 80, %custom_map_left%, 140, 0xFFDD44, 2, Fast RGB        ;left side
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
			}	
			if (navigation = 1 and Outputvar = "custom map (large)")
			{
				custom_map_left_long = % slidermapL.pos + 8
				y_value := slidermapR.pos
				custom_map_right_long := 288 - y_value
			
				PixelSearch,,, %custom_map_right_long%, 136, 226, 157, 0xFFDD44, 2, Fast RGB     ;right side
				if !ErrorLevel
				{
					Sendinput {Blind}{Right up}
					Sendinput {Left down}
				}
		
				PixelSearch,,, 8, 136, %custom_map_left_long%, 157, 0xFFDD44, 2, Fast RGB        ;left side
				if !ErrorLevel
				{
					Sendinput {Blind}{Left up}
					Sendinput {Right down}
				}
			}
		}
	}	
return


;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> functions

Single_Jump()
{
	Sendinput {Alt down}
	Sleep 100ms
	Sendinput {Blind}{Alt up}
}

Double_Jump_Fast()
{
	Sendinput {Alt down}
	Sleep 100ms
	Sendinput {Blind}{Alt up}
	Sendinput {Alt down}
	Sleep 100ms
	Sendinput {Blind}{Alt up}
}

Double_Jump_Slow()
{
	Sendinput {Alt down}
	Sleep 300ms
	Sendinput {Blind}{Alt up}
	Sendinput {Alt down}
	Sleep 300ms
	Sendinput {Blind}{Alt up}
}

Teleport()
{
	GuiControlGet, Movementvar_key, ,DDLmovement_key,
	Sendinput {%Movementvar_key%}
}

;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> keybindings

*F11::

return


*F6::
buttonnav_switch:
togglenavigation:=!togglenavigation

	if (!togglenavigation)
	{
		Sendinput {Blind}{Left up}
		Sendinput {Blind}{Right up}
		Sendinput {Blind}{Up up}
		Sendinput {Blind}{Down up}
		GuiControl,, buttonnav,  % "HBITMAP:*" . ui_buttonoff_bitmap
		navigation = 0
		rune_execute = 0
		looping_arrows = 0
	}
	else
	{
		GuiControl,, buttonnav, % "HBITMAP:*" . ui_buttonon_bitmap
		navigation = 1
	}
return

buttonrune_toggle:
togglerune:=!togglerune

	if (!togglerune)
	{
		Sendinput {Blind}{Left up}
		Sendinput {Blind}{Right up}
		Sendinput {Blind}{Up up}
		Sendinput {Blind}{Down up}
		GuiControl,, buttonrune,  % "HBITMAP:*" . ui_buttonoff_bitmap
		autorune = 0
	}
	else
	{
		GuiControl,, buttonrune, % "HBITMAP:*" . ui_buttonon_bitmap
		autorune = 1
	}
return

*F7::
buttonmovement_switch:
togglemovement:=!togglemovement

	if (!togglemovement)
	{
		GuiControl,, buttonmovement,  % "HBITMAP:*" . ui_buttonoff_bitmap
		movement = 0
		Pause
	}
	else
	{
		GuiControl,, buttonmovement, % "HBITMAP:*" . ui_buttonon_bitmap

		Loop {
			if (togglemovement) {
				
/*
				Loop, 1 {
					SendInput {Down Down}
					Sleep, 300
					SendInput {d}
					SendInput {Down Up}
				}
					*/
				Loop, 1 {
					SendInput {Left Down}
					Sleep, %Delayvar%
					SendInput {Left Up}
				}
				Loop, 1 {
					SendInput {Right Down}
					Sleep, %Delayvar%
					SendInput {Right Up}
				}
/*

				Loop, 1 {
					SendInput {Up Down}
					SendInput {d}
					Sleep, 10
					SendInput {Up Up}
				}
*/				

			} else {
				break
			}
		}
	
		movement = 1
	}
return

; script commands
^r::Reload
^p::Pause
^z::ExitApp, [ ExitCode]

buttonontop_switch:
toggleontop:=!toggleontop				
	if (!toggleontop)
	{

		Gui, -AlwaysOnTop
		GuiControl,, buttonontop, % "HBITMAP:*" . ui_buttonoff_bitmap
	}
	else
	{
		GuiControl,, buttonontop, % "HBITMAP:*" . ui_buttonon_bitmap
		Gui, +AlwaysOnTop
	}
return


button_dc:
toggledc:=!toggledc			
	if (!toggledc)
	{
		GuiControl,, buttondc, % "HBITMAP:*" . ui_buttonoff_bitmap
		dc = 0
	}
	else
	{
		GuiControl,, buttondc, % "HBITMAP:*" . ui_buttonon_bitmap
		dc = 1 
	}
return

;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> Button toggle down images

buttontoggle_down:
GuiControl,,gui_button_toggle, % "HBITMAP:*" . ui_buttontoggle_down_bitmap
	Loop
	{
		LM:=GetKeyState("LButton")
		if(LM=False)
		{
			break
		}
	}
GuiControl,,gui_button_toggle, % "HBITMAP:*" . ui_buttontoggle_bitmap

;Process, Exist, omes.skills.exe
Process, Exist, omes.skills.ahk
	if !ErrorLevel
	{
		;Run, omes.skills.exe
		Run, omes.skills.ahk
	}
	else if ErrorLevel = 1
	{
		msgbox Omes.skills.exe not found!
	}
return

;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> Button toggle autopot ui

buttonautopot_down:
GuiControl,,gui_button_autopot, % "HBITMAP:*" . ui_button_autopot_down_bitmap
	Loop
	{
		LM:=GetKeyState("LButton")
		if(LM=False)
		{
			break
		}
	}
GuiControl,,gui_button_autopot, % "HBITMAP:*" . ui_button_autopot_bitmap

;Process, Exist, omes.skills.exe
Process, Exist, omes.autopot.ahk
	if !ErrorLevel
	{
		;Run, omes.skills.exe
		Run, omes.autopot.ahk
	}
	else if ErrorLevel = 1
	{
		msgbox Omes.autopot.exe not found!
	}
return


;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> buttonkillswitch

;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> buttonkillswitch

buttonkill_down:

		LM:=GetKeyState("LButton")
		Sendinput {Blind}{Alt up}
		Sendinput {Blind}{Space up}
		Sendinput {Blind}{Control up}
		Sendinput {Blind}{Up up}
		Sendinput {Blind}{Down up}
		Sendinput {Blind}{Left up}
		Sendinput {Blind}{Right up}
		
		GuiControl,, buttonautopot,  % "HBITMAP:*" . ui_buttonoff_bitmap
		potion = 0
		
		GuiControl,, buttonmovement,  % "HBITMAP:*" . ui_buttonoff_bitmap
		movement = 0
		
		GuiControl,, buttonnav,  % "HBITMAP:*" . ui_buttonoff_bitmap
		navigation = 0
		

GuiControl,,gui_button_close, % "HBITMAP:*" . ui_buttonkillswitch_down_bitmap
	Loop
	{
		LM:=GetKeyState("LButton")
		if(LM=False)
		{
			break
		}
	}
GuiControl,,gui_button_close, % "HBITMAP:*" . ui_buttonkillswitch_bitmap

return
;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>



guiclose:
ExitApp
return






































class CustomSlider_reverse
{
	__New(Options := "", val := "", hBitmap_thumb := "", ShowTooltip := false) {
		if !RegExMatch(Options, "i)\bBackground\w+\b") {                                      ;i)turns off case-sensitivity
			if RegExMatch(Options, "i)\bc\K\w+", bkColor)
				bkOpt := "Background" bkColor
		}
		Gui, Add, Progress, h1 %Options% %bkOpt% hwndHPROG Disabled +E0x00400000, % val       ;-E0x200  Left to right    +E0x00400000 triggers backwards progress

		Gui, Add, Text, xp yp-10 h24 wp BackgroundTrans HWNDhpgTrigger,  ;determins slider position

		if RegExMatch(Options, "i)\bRange\K([\d-]+)-([\d-]+)", rng)
			this.rng_radio := Abs(rng2 - rng1)/100
		else
			this.rng_radio := 1

		GuiControlGet, pg, Pos, %HPROG%
		x := pgW * (val/this.rng_radio/100) + pgX

		Gui, Add, Pic, x%x% yp w8 h-20 HWNDhBtn, % "HBITMAP:" hBitmap_thumb  ;creates the ui for the slider and input ??? gui adds the slider0
		this.hProg := HPROG
		this.hBtn := hBtn
		this.pgVal := val
		this.ShowTooltip := ShowTooltip
		; this.hRightText := hRightText

		fn := this.OnClick_reverse.Bind(this)
		GuiControl, +g, %hBtn%, %fn%
		GuiControl, +g, %hpgTrigger%, %fn%
	}

	OnClick_reverse() {
		GuiControl, Focus, % this.hBtn

		hSlider := this.hProg

		pre_CoordModeMouse := A_CoordModeMouse
		CoordMode, Mouse, Relative

		MouseGetPos,,,, ClickedhWnd, 2

		GuiControlGet, SliderLine, %A_Gui%:Pos, % hSlider

		GuiControlGet, sliderVal, %A_Gui%:, %hSlider%
		V := sliderVal

		if this.ShowTooltip
			ToolTip, %sliderVal%

		ControlGet, Style, Style,,, ahk_id %hSlider%
		ControlGetPos, X, Y, W, H,, ahk_id %hSlider%

		VarSetCapacity(R, 8)
		SendMessage, 0x0407,, &R,, ahk_id %hSlider%
		R1 := NumGet(R, 4, "Int")
		R2 := NumGet(R, 0, "Int")
		
		LastV := (ClickedhWnd = this.hBtn) ? "" : sliderVal

		while (GetKeyState("LButton"))
		{
			Sleep, 10
			MouseGetPos, XM, YM
			V := (V:=(Style&0x4 ? 1-(YM-Y)/H : (XM-X)/W))>=1 ? R2 : V<=0 ? R1 : Round(V*(R2-R1)+R1)
			if (LastV="") {
				LastV := V
			} else if (V != LastV) {
				LastV := V
				if this.ShowTooltip
					ToolTip % V
				this.pos(SliderLineW, SliderLineX) := V
			}
		}

		if this.ShowTooltip
			ToolTip
		CoordMode, Mouse, %pre_CoordModeMouse%
	}

	pos[pgW := "", pgX := ""] {
		set {
			GuiControl,, % this.hProg, % value

			if !pgW
				GuiControlGet, pg, Pos, % this.hProg

			x := pgW * (value/this.rng_radio/100) + pgX
			; GuiControl, MoveDraw, % this.hBtn, x%x%
			GuiControl, Move, % this.hBtn, x%x%
			DllCall("InvalidateRect", "ptr", this.hBtn, "ptr", 0, "int", 0)

			this.pgVal := value
			; GuiControl,, % this.hRightText, % value
		}
		get {
			return this.pgVal
		}
	}
}













; based on http://www.autohotkey.com/board/topic/81144-progress-bar-slider/?p=516891
class CustomSlider
{
	__New(Options := "", val := "", hBitmap_thumb := "", ShowTooltip := false) {
		if !RegExMatch(Options, "i)\bBackground\w+\b") {                                      ;i)turns off case-sensitivity
			if RegExMatch(Options, "i)\bc\K\w+", bkColor)
				bkOpt := "Background" bkColor
		}
		Gui, Add, Progress, h1 %Options% %bkOpt% hwndHPROG Disabled -E0x200  , % val

		Gui, Add, Text, xp yp-10 h24 wp BackgroundTrans HWNDhpgTrigger,  ;determins slider position

		if RegExMatch(Options, "i)\bRange\K([\d-]+)-([\d-]+)", rng)
			this.rng_radio := Abs(rng2 - rng1)/100
		else
			this.rng_radio := 1

		GuiControlGet, pg, Pos, %HPROG%
		x := pgW * (val/this.rng_radio/100) + pgX

		;Gui, Add, Pic, xp yp w15 h29 HWNDhBtn  ;creates the ui for the slider and input ??? gui adds the slider
		Gui, Add, Pic, yp x%x% w8 h-20 HWNDhBtn, % "HBITMAP:*" hBitmap_thumb  ;creates the ui for the slider and input ??? gui adds the slider

		this.hProg := HPROG
		this.hBtn := hBtn
		this.pgVal := val
		this.ShowTooltip := ShowTooltip
		; this.hRightText := hRightText

		fn := this.OnClick.Bind(this)
		GuiControl, +g, %hBtn%, %fn%
		GuiControl, +g, %hpgTrigger%, %fn%
	}

	OnClick() {
		GuiControl, Focus, % this.hBtn

		hSlider := this.hProg

		pre_CoordModeMouse := A_CoordModeMouse
		CoordMode, Mouse, Relative

		MouseGetPos,,,, ClickedhWnd, 2

		GuiControlGet, SliderLine, %A_Gui%:Pos, % hSlider

		GuiControlGet, sliderVal, %A_Gui%:, %hSlider%
		V := sliderVal

		if this.ShowTooltip
			ToolTip, %sliderVal%

		ControlGet, Style, Style,,, ahk_id %hSlider%
		ControlGetPos, X, Y, W, H,, ahk_id %hSlider%

		VarSetCapacity(R, 8)
		SendMessage, 0x0407,, &R,, ahk_id %hSlider%
		R1 := NumGet(R, 0, "Int")
		R2 := NumGet(R, 4, "Int")
		
		LastV := (ClickedhWnd = this.hBtn) ? "" : sliderVal

		while (GetKeyState("LButton"))
		{
			Sleep, 10
			MouseGetPos, XM, YM
			V := (V:=(Style&0x4 ? 1-(YM-Y)/H : (XM-X)/W))>=1 ? R2 : V<=0 ? R1 : Round(V*(R2-R1)+R1)
			if (LastV="") {
				LastV := V
			} else if (V != LastV) {
				LastV := V
				if this.ShowTooltip
					ToolTip % V
				this.pos(SliderLineW, SliderLineX) := V
			}
		}

		if this.ShowTooltip
			ToolTip
		CoordMode, Mouse, %pre_CoordModeMouse%
	}

	pos[pgW := "", pgX := ""] {
		set {
			GuiControl,, % this.hProg, % value

			if !pgW
				GuiControlGet, pg, Pos, % this.hProg

			x := pgW * (value/this.rng_radio/100) + pgX
			; GuiControl, MoveDraw, % this.hBtn, x%x%
			GuiControl, Move, % this.hBtn, x%x%
			DllCall("InvalidateRect", "ptr", this.hBtn, "ptr", 0, "int", 0)

			this.pgVal := value
			; GuiControl,, % this.hRightText, % value
		}
		get {
			return this.pgVal
		}
	}
}


; ##################################################################################
; # This #Include file was generated by Image2Include.ahk, you must not change it! #
; ##################################################################################
Create_buttonautopot_png(NewHandle = False) {
Static hBitmap := 0
Ptr := A_PtrSize ? "Ptr" : "UInt"
UPtr := A_PtrSize ? "UPtr" : "UInt"
If (NewHandle)
   hBitmap := 0
If (hBitmap)
   Return hBitmap
VarSetCapacity(B64, 6016 << !!A_IsUnicode)
B64 := ""
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, 0, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
VarSetCapacity(Dec, DecLen, 0)
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, &Dec, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
; Bitmap creation adopted from "How to convert Image data (JPEG/PNG/GIF) to hBITMAP?" by SKAN
; -> http://www.autohotkey.com/board/topic/21213-how-to-convert-image-data-jpegpnggif-to-hbitmap/?p=139257
hData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2, UPtr, DecLen, UPtr)
pData := DllCall("Kernel32.dll\GlobalLock", Ptr, hData, UPtr)
DllCall("Kernel32.dll\RtlMoveMemory", Ptr, pData, Ptr, &Dec, UPtr, DecLen)
DllCall("Kernel32.dll\GlobalUnlock", Ptr, hData)
DllCall("Ole32.dll\CreateStreamOnHGlobal", Ptr, hData, "Int", True, Ptr "P", pStream)
hGdip := DllCall("Kernel32.dll\LoadLibrary", "Str", "Gdiplus.dll", UPtr)
VarSetCapacity(SI, 16, 0), NumPut(1, SI, 0, "UChar")
DllCall("Gdiplus.dll\GdiplusStartup", Ptr "P", pToken, Ptr, &SI, Ptr, 0)
DllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  Ptr, pStream, Ptr "P", pBitmap)
DllCall("Gdiplus.dll\GdipCreateHBITMAPFromBitmap", Ptr, pBitmap, Ptr "P", hBitmap, "UInt", 0)
DllCall("Gdiplus.dll\GdipDisposeImage", Ptr, pBitmap)
DllCall("Gdiplus.dll\GdiplusShutdown", Ptr, pToken)
DllCall("Kernel32.dll\FreeLibrary", Ptr, hGdip)
PtrSize := A_PtrSize ? A_PtrSize : 4
DllCall(NumGet(NumGet(pStream + 0, 0, UPtr) + (PtrSize * 2), 0, UPtr), Ptr, pStream)
Return hBitmap
}

; ##################################################################################
; # This #Include file was generated by Image2Include.ahk, you must not change it! #
; ##################################################################################
Create_buttonautopotdown_png(NewHandle = False) {
Static hBitmap := 0
Ptr := A_PtrSize ? "Ptr" : "UInt"
UPtr := A_PtrSize ? "UPtr" : "UInt"
If (NewHandle)
   hBitmap := 0
If (hBitmap)
   Return hBitmap
VarSetCapacity(B64, 5972 << !!A_IsUnicode)
B64 := ""
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, 0, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
VarSetCapacity(Dec, DecLen, 0)
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, &Dec, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
; Bitmap creation adopted from "How to convert Image data (JPEG/PNG/GIF) to hBITMAP?" by SKAN
; -> http://www.autohotkey.com/board/topic/21213-how-to-convert-image-data-jpegpnggif-to-hbitmap/?p=139257
hData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2, UPtr, DecLen, UPtr)
pData := DllCall("Kernel32.dll\GlobalLock", Ptr, hData, UPtr)
DllCall("Kernel32.dll\RtlMoveMemory", Ptr, pData, Ptr, &Dec, UPtr, DecLen)
DllCall("Kernel32.dll\GlobalUnlock", Ptr, hData)
DllCall("Ole32.dll\CreateStreamOnHGlobal", Ptr, hData, "Int", True, Ptr "P", pStream)
hGdip := DllCall("Kernel32.dll\LoadLibrary", "Str", "Gdiplus.dll", UPtr)
VarSetCapacity(SI, 16, 0), NumPut(1, SI, 0, "UChar")
DllCall("Gdiplus.dll\GdiplusStartup", Ptr "P", pToken, Ptr, &SI, Ptr, 0)
DllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  Ptr, pStream, Ptr "P", pBitmap)
DllCall("Gdiplus.dll\GdipCreateHBITMAPFromBitmap", Ptr, pBitmap, Ptr "P", hBitmap, "UInt", 0)
DllCall("Gdiplus.dll\GdipDisposeImage", Ptr, pBitmap)
DllCall("Gdiplus.dll\GdiplusShutdown", Ptr, pToken)
DllCall("Kernel32.dll\FreeLibrary", Ptr, hGdip)
PtrSize := A_PtrSize ? A_PtrSize : 4
DllCall(NumGet(NumGet(pStream + 0, 0, UPtr) + (PtrSize * 2), 0, UPtr), Ptr, pStream)
Return hBitmap
}


; ##################################################################################
; # This #Include file was generated by Image2Include.ahk, you must not change it! #
; ##################################################################################
Create_buttonon_png1(NewHandle = False) {
Static hBitmap := 0
Ptr := A_PtrSize ? "Ptr" : "UInt"
UPtr := A_PtrSize ? "UPtr" : "UInt"
If (NewHandle)
   hBitmap := 0
If (hBitmap)
   Return hBitmap
VarSetCapacity(B64, 3824 << !!A_IsUnicode)
B64 := ""
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, 0, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
VarSetCapacity(Dec, DecLen, 0)
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, &Dec, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
; Bitmap creation adopted from "How to convert Image data (JPEG/PNG/GIF) to hBITMAP?" by SKAN
; -> http://www.autohotkey.com/board/topic/21213-how-to-convert-image-data-jpegpnggif-to-hbitmap/?p=139257
hData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2, UPtr, DecLen, UPtr)
pData := DllCall("Kernel32.dll\GlobalLock", Ptr, hData, UPtr)
DllCall("Kernel32.dll\RtlMoveMemory", Ptr, pData, Ptr, &Dec, UPtr, DecLen)
DllCall("Kernel32.dll\GlobalUnlock", Ptr, hData)
DllCall("Ole32.dll\CreateStreamOnHGlobal", Ptr, hData, "Int", True, Ptr "P", pStream)
hGdip := DllCall("Kernel32.dll\LoadLibrary", "Str", "Gdiplus.dll", UPtr)
VarSetCapacity(SI, 16, 0), NumPut(1, SI, 0, "UChar")
DllCall("Gdiplus.dll\GdiplusStartup", Ptr "P", pToken, Ptr, &SI, Ptr, 0)
DllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  Ptr, pStream, Ptr "P", pBitmap)
DllCall("Gdiplus.dll\GdipCreateHBITMAPFromBitmap", Ptr, pBitmap, Ptr "P", hBitmap, "UInt", 0)
DllCall("Gdiplus.dll\GdipDisposeImage", Ptr, pBitmap)
DllCall("Gdiplus.dll\GdiplusShutdown", Ptr, pToken)
DllCall("Kernel32.dll\FreeLibrary", Ptr, hGdip)
PtrSize := A_PtrSize ? A_PtrSize : 4
DllCall(NumGet(NumGet(pStream + 0, 0, UPtr) + (PtrSize * 2), 0, UPtr), Ptr, pStream)
Return hBitmap
}





;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> UI
;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> UI
;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> UI
;<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> UI





; ##################################################################################
; # This #Include file was generated by Image2Include.ahk, you must not change it! #
; ##################################################################################
Create_buttonon_png(NewHandle = False) {
Static hBitmap := 0
Ptr := A_PtrSize ? "Ptr" : "UInt"
UPtr := A_PtrSize ? "UPtr" : "UInt"
If (NewHandle)
   hBitmap := 0
If (hBitmap)
   Return hBitmap
VarSetCapacity(B64, 3824 << !!A_IsUnicode)
B64 := ""
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, 0, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
VarSetCapacity(Dec, DecLen, 0)
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, &Dec, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
; Bitmap creation adopted from "How to convert Image data (JPEG/PNG/GIF) to hBITMAP?" by SKAN
; -> http://www.autohotkey.com/board/topic/21213-how-to-convert-image-data-jpegpnggif-to-hbitmap/?p=139257
hData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2, UPtr, DecLen, UPtr)
pData := DllCall("Kernel32.dll\GlobalLock", Ptr, hData, UPtr)
DllCall("Kernel32.dll\RtlMoveMemory", Ptr, pData, Ptr, &Dec, UPtr, DecLen)
DllCall("Kernel32.dll\GlobalUnlock", Ptr, hData)
DllCall("Ole32.dll\CreateStreamOnHGlobal", Ptr, hData, "Int", True, Ptr "P", pStream)
hGdip := DllCall("Kernel32.dll\LoadLibrary", "Str", "Gdiplus.dll", UPtr)
VarSetCapacity(SI, 16, 0), NumPut(1, SI, 0, "UChar")
DllCall("Gdiplus.dll\GdiplusStartup", Ptr "P", pToken, Ptr, &SI, Ptr, 0)
DllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  Ptr, pStream, Ptr "P", pBitmap)
DllCall("Gdiplus.dll\GdipCreateHBITMAPFromBitmap", Ptr, pBitmap, Ptr "P", hBitmap, "UInt", 0)
DllCall("Gdiplus.dll\GdipDisposeImage", Ptr, pBitmap)
DllCall("Gdiplus.dll\GdiplusShutdown", Ptr, pToken)
DllCall("Kernel32.dll\FreeLibrary", Ptr, hGdip)
PtrSize := A_PtrSize ? A_PtrSize : 4
DllCall(NumGet(NumGet(pStream + 0, 0, UPtr) + (PtrSize * 2), 0, UPtr), Ptr, pStream)
Return hBitmap
}



; ##################################################################################
; # This #Include file was generated by Image2Include.ahk, you must not change it! #
; ##################################################################################
Create_buttonoff_png(NewHandle = False) {
Static hBitmap := 0
Ptr := A_PtrSize ? "Ptr" : "UInt"
UPtr := A_PtrSize ? "UPtr" : "UInt"
If (NewHandle)
   hBitmap := 0
If (hBitmap)
   Return hBitmap
VarSetCapacity(B64, 3692 << !!A_IsUnicode)
B64 := ""
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, 0, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
VarSetCapacity(Dec, DecLen, 0)
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, &Dec, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
; Bitmap creation adopted from "How to convert Image data (JPEG/PNG/GIF) to hBITMAP?" by SKAN
; -> http://www.autohotkey.com/board/topic/21213-how-to-convert-image-data-jpegpnggif-to-hbitmap/?p=139257
hData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2, UPtr, DecLen, UPtr)
pData := DllCall("Kernel32.dll\GlobalLock", Ptr, hData, UPtr)
DllCall("Kernel32.dll\RtlMoveMemory", Ptr, pData, Ptr, &Dec, UPtr, DecLen)
DllCall("Kernel32.dll\GlobalUnlock", Ptr, hData)
DllCall("Ole32.dll\CreateStreamOnHGlobal", Ptr, hData, "Int", True, Ptr "P", pStream)
hGdip := DllCall("Kernel32.dll\LoadLibrary", "Str", "Gdiplus.dll", UPtr)
VarSetCapacity(SI, 16, 0), NumPut(1, SI, 0, "UChar")
DllCall("Gdiplus.dll\GdiplusStartup", Ptr "P", pToken, Ptr, &SI, Ptr, 0)
DllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  Ptr, pStream, Ptr "P", pBitmap)
DllCall("Gdiplus.dll\GdipCreateHBITMAPFromBitmap", Ptr, pBitmap, Ptr "P", hBitmap, "UInt", 0)
DllCall("Gdiplus.dll\GdipDisposeImage", Ptr, pBitmap)
DllCall("Gdiplus.dll\GdiplusShutdown", Ptr, pToken)
DllCall("Kernel32.dll\FreeLibrary", Ptr, hGdip)
PtrSize := A_PtrSize ? A_PtrSize : 4
DllCall(NumGet(NumGet(pStream + 0, 0, UPtr) + (PtrSize * 2), 0, UPtr), Ptr, pStream)
Return hBitmap
}
; ##################################################################################
; # This #Include file was generated by Image2Include.ahk, you must not change it! #
; ##################################################################################

Create_uiupdate_png(NewHandle = False) {
Static hBitmap := 0
Ptr := A_PtrSize ? "Ptr" : "UInt"
UPtr := A_PtrSize ? "UPtr" : "UInt"
If (NewHandle)
   hBitmap := 0
If (hBitmap)
   Return hBitmap
VarSetCapacity(B64, 19644 << !!A_IsUnicode)
B64 := ""
B64 .= ""
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, 0, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
VarSetCapacity(Dec, DecLen, 0)
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, &Dec, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
; Bitmap creation adopted from "How to convert Image data (JPEG/PNG/GIF) to hBITMAP?" by SKAN
; -> http://www.autohotkey.com/board/topic/21213-how-to-convert-image-data-jpegpnggif-to-hbitmap/?p=139257
hData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2, UPtr, DecLen, UPtr)
pData := DllCall("Kernel32.dll\GlobalLock", Ptr, hData, UPtr)
DllCall("Kernel32.dll\RtlMoveMemory", Ptr, pData, Ptr, &Dec, UPtr, DecLen)
DllCall("Kernel32.dll\GlobalUnlock", Ptr, hData)
DllCall("Ole32.dll\CreateStreamOnHGlobal", Ptr, hData, "Int", True, Ptr "P", pStream)
hGdip := DllCall("Kernel32.dll\LoadLibrary", "Str", "Gdiplus.dll", UPtr)
VarSetCapacity(SI, 16, 0), NumPut(1, SI, 0, "UChar")
DllCall("Gdiplus.dll\GdiplusStartup", Ptr "P", pToken, Ptr, &SI, Ptr, 0)
DllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  Ptr, pStream, Ptr "P", pBitmap)
DllCall("Gdiplus.dll\GdipCreateHBITMAPFromBitmap", Ptr, pBitmap, Ptr "P", hBitmap, "UInt", 0)
DllCall("Gdiplus.dll\GdipDisposeImage", Ptr, pBitmap)
DllCall("Gdiplus.dll\GdiplusShutdown", Ptr, pToken)
DllCall("Kernel32.dll\FreeLibrary", Ptr, hGdip)
PtrSize := A_PtrSize ? A_PtrSize : 4
DllCall(NumGet(NumGet(pStream + 0, 0, UPtr) + (PtrSize * 2), 0, UPtr), Ptr, pStream)
Return hBitmap
}



; ##################################################################################
; # This #Include file was generated by Image2Include.ahk, you must not change it! #
; ##################################################################################
Create_buttonkillswitch_png(NewHandle = False) {
Static hBitmap := 0
Ptr := A_PtrSize ? "Ptr" : "UInt"
UPtr := A_PtrSize ? "UPtr" : "UInt"
If (NewHandle)
   hBitmap := 0
If (hBitmap)
   Return hBitmap
VarSetCapacity(B64, 6220 << !!A_IsUnicode)
B64 := ""
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, 0, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
VarSetCapacity(Dec, DecLen, 0)
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, &Dec, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
; Bitmap creation adopted from "How to convert Image data (JPEG/PNG/GIF) to hBITMAP?" by SKAN
; -> http://www.autohotkey.com/board/topic/21213-how-to-convert-image-data-jpegpnggif-to-hbitmap/?p=139257
hData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2, UPtr, DecLen, UPtr)
pData := DllCall("Kernel32.dll\GlobalLock", Ptr, hData, UPtr)
DllCall("Kernel32.dll\RtlMoveMemory", Ptr, pData, Ptr, &Dec, UPtr, DecLen)
DllCall("Kernel32.dll\GlobalUnlock", Ptr, hData)
DllCall("Ole32.dll\CreateStreamOnHGlobal", Ptr, hData, "Int", True, Ptr "P", pStream)
hGdip := DllCall("Kernel32.dll\LoadLibrary", "Str", "Gdiplus.dll", UPtr)
VarSetCapacity(SI, 16, 0), NumPut(1, SI, 0, "UChar")
DllCall("Gdiplus.dll\GdiplusStartup", Ptr "P", pToken, Ptr, &SI, Ptr, 0)
DllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  Ptr, pStream, Ptr "P", pBitmap)
DllCall("Gdiplus.dll\GdipCreateHBITMAPFromBitmap", Ptr, pBitmap, Ptr "P", hBitmap, "UInt", 0)
DllCall("Gdiplus.dll\GdipDisposeImage", Ptr, pBitmap)
DllCall("Gdiplus.dll\GdiplusShutdown", Ptr, pToken)
DllCall("Kernel32.dll\FreeLibrary", Ptr, hGdip)
PtrSize := A_PtrSize ? A_PtrSize : 4
DllCall(NumGet(NumGet(pStream + 0, 0, UPtr) + (PtrSize * 2), 0, UPtr), Ptr, pStream)
Return hBitmap
}

; ##################################################################################
; # This #Include file was generated by Image2Include.ahk, you must not change it! #
; ##################################################################################
Create_buttonkillswitchdown_png(NewHandle = False) {
Static hBitmap := 0
Ptr := A_PtrSize ? "Ptr" : "UInt"
UPtr := A_PtrSize ? "UPtr" : "UInt"
If (NewHandle)
   hBitmap := 0
If (hBitmap)
   Return hBitmap
VarSetCapacity(B64, 6132 << !!A_IsUnicode)
B64 := ""
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, 0, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
VarSetCapacity(Dec, DecLen, 0)
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, &Dec, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
; Bitmap creation adopted from "How to convert Image data (JPEG/PNG/GIF) to hBITMAP?" by SKAN
; -> http://www.autohotkey.com/board/topic/21213-how-to-convert-image-data-jpegpnggif-to-hbitmap/?p=139257
hData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2, UPtr, DecLen, UPtr)
pData := DllCall("Kernel32.dll\GlobalLock", Ptr, hData, UPtr)
DllCall("Kernel32.dll\RtlMoveMemory", Ptr, pData, Ptr, &Dec, UPtr, DecLen)
DllCall("Kernel32.dll\GlobalUnlock", Ptr, hData)
DllCall("Ole32.dll\CreateStreamOnHGlobal", Ptr, hData, "Int", True, Ptr "P", pStream)
hGdip := DllCall("Kernel32.dll\LoadLibrary", "Str", "Gdiplus.dll", UPtr)
VarSetCapacity(SI, 16, 0), NumPut(1, SI, 0, "UChar")
DllCall("Gdiplus.dll\GdiplusStartup", Ptr "P", pToken, Ptr, &SI, Ptr, 0)
DllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  Ptr, pStream, Ptr "P", pBitmap)
DllCall("Gdiplus.dll\GdipCreateHBITMAPFromBitmap", Ptr, pBitmap, Ptr "P", hBitmap, "UInt", 0)
DllCall("Gdiplus.dll\GdipDisposeImage", Ptr, pBitmap)
DllCall("Gdiplus.dll\GdiplusShutdown", Ptr, pToken)
DllCall("Kernel32.dll\FreeLibrary", Ptr, hGdip)
PtrSize := A_PtrSize ? A_PtrSize : 4
DllCall(NumGet(NumGet(pStream + 0, 0, UPtr) + (PtrSize * 2), 0, UPtr), Ptr, pStream)
Return hBitmap
}


; ##################################################################################
; # This #Include file was generated by Image2Include.ahk, you must not change it! #
; ##################################################################################
Create_buttontoggle_png(NewHandle = False) {
Static hBitmap := 0
Ptr := A_PtrSize ? "Ptr" : "UInt"
UPtr := A_PtrSize ? "UPtr" : "UInt"
If (NewHandle)
   hBitmap := 0
If (hBitmap)
   Return hBitmap
VarSetCapacity(B64, 5728 << !!A_IsUnicode)
B64 := ""
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, 0, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
VarSetCapacity(Dec, DecLen, 0)
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, &Dec, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
; Bitmap creation adopted from "How to convert Image data (JPEG/PNG/GIF) to hBITMAP?" by SKAN
; -> http://www.autohotkey.com/board/topic/21213-how-to-convert-image-data-jpegpnggif-to-hbitmap/?p=139257
hData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2, UPtr, DecLen, UPtr)
pData := DllCall("Kernel32.dll\GlobalLock", Ptr, hData, UPtr)
DllCall("Kernel32.dll\RtlMoveMemory", Ptr, pData, Ptr, &Dec, UPtr, DecLen)
DllCall("Kernel32.dll\GlobalUnlock", Ptr, hData)
DllCall("Ole32.dll\CreateStreamOnHGlobal", Ptr, hData, "Int", True, Ptr "P", pStream)
hGdip := DllCall("Kernel32.dll\LoadLibrary", "Str", "Gdiplus.dll", UPtr)
VarSetCapacity(SI, 16, 0), NumPut(1, SI, 0, "UChar")
DllCall("Gdiplus.dll\GdiplusStartup", Ptr "P", pToken, Ptr, &SI, Ptr, 0)
DllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  Ptr, pStream, Ptr "P", pBitmap)
DllCall("Gdiplus.dll\GdipCreateHBITMAPFromBitmap", Ptr, pBitmap, Ptr "P", hBitmap, "UInt", 0)
DllCall("Gdiplus.dll\GdipDisposeImage", Ptr, pBitmap)
DllCall("Gdiplus.dll\GdiplusShutdown", Ptr, pToken)
DllCall("Kernel32.dll\FreeLibrary", Ptr, hGdip)
PtrSize := A_PtrSize ? A_PtrSize : 4
DllCall(NumGet(NumGet(pStream + 0, 0, UPtr) + (PtrSize * 2), 0, UPtr), Ptr, pStream)
Return hBitmap
}


; ##################################################################################
; # This #Include file was generated by Image2Include.ahk, you must not change it! #
; ##################################################################################
Create_buttontoggledown_png(NewHandle = False) {
Static hBitmap := 0
Ptr := A_PtrSize ? "Ptr" : "UInt"
UPtr := A_PtrSize ? "UPtr" : "UInt"
If (NewHandle)
   hBitmap := 0
If (hBitmap)
   Return hBitmap
VarSetCapacity(B64, 5684 << !!A_IsUnicode)
B64 := ""
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, 0, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
VarSetCapacity(Dec, DecLen, 0)
If !DllCall("Crypt32.dll\CryptStringToBinary" (A_IsUnicode ? "W" : "A"), Ptr, &B64, "UInt", 0, "UInt", 0x01, Ptr, &Dec, "UIntP", DecLen, Ptr, 0, Ptr, 0)
   Return False
; Bitmap creation adopted from "How to convert Image data (JPEG/PNG/GIF) to hBITMAP?" by SKAN
; -> http://www.autohotkey.com/board/topic/21213-how-to-convert-image-data-jpegpnggif-to-hbitmap/?p=139257
hData := DllCall("Kernel32.dll\GlobalAlloc", "UInt", 2, UPtr, DecLen, UPtr)
pData := DllCall("Kernel32.dll\GlobalLock", Ptr, hData, UPtr)
DllCall("Kernel32.dll\RtlMoveMemory", Ptr, pData, Ptr, &Dec, UPtr, DecLen)
DllCall("Kernel32.dll\GlobalUnlock", Ptr, hData)
DllCall("Ole32.dll\CreateStreamOnHGlobal", Ptr, hData, "Int", True, Ptr "P", pStream)
hGdip := DllCall("Kernel32.dll\LoadLibrary", "Str", "Gdiplus.dll", UPtr)
VarSetCapacity(SI, 16, 0), NumPut(1, SI, 0, "UChar")
DllCall("Gdiplus.dll\GdiplusStartup", Ptr "P", pToken, Ptr, &SI, Ptr, 0)
DllCall("Gdiplus.dll\GdipCreateBitmapFromStream",  Ptr, pStream, Ptr "P", pBitmap)
DllCall("Gdiplus.dll\GdipCreateHBITMAPFromBitmap", Ptr, pBitmap, Ptr "P", hBitmap, "UInt", 0)
DllCall("Gdiplus.dll\GdipDisposeImage", Ptr, pBitmap)
DllCall("Gdiplus.dll\GdiplusShutdown", Ptr, pToken)
DllCall("Kernel32.dll\FreeLibrary", Ptr, hGdip)
PtrSize := A_PtrSize ? A_PtrSize : 4
DllCall(NumGet(NumGet(pStream + 0, 0, UPtr) + (PtrSize * 2), 0, UPtr), Ptr, pStream)
Return hBitmap
}
